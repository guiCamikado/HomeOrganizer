...
function RegisterCard() {
    ...
    async function sendRegistrationRequest() {
        if (passNotEqual || termos) {
            ...
            try {
                const response = await request.post('http://127.0.0.1:5000/DBAction/newRegisterRequest', data);
            }
            catch (error) {
                console.error("Erro ao registrar usuário:", error);
            }
        } else {
            alert("Por favor, verifique se as senhas coincidem e se você aceitou os termos de uso.");
            return;
        }
    }
            ...
            <div className="pt-16 flex flex-row justify-center items-center">
                <div className="border w-2xl h-2x1 bg-stone-950/60 p-6 text-amber-50 rounded-4xl">
                    <Form>
                        ...
                        <div className="flex">
                            <Button type="submit" className={`${buttonClass} mr-2`} onClick={sendRegistrationRequest}>
                                Registrar
                            </Button>
